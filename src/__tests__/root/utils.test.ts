...
// ── hasLLMConfig ─────────────────────────────────────────────────────────
//
// Drives requireLLMReady() in main.ts. The function decides whether the user
// has supplied enough configuration (provider + apiKey + model, with local
// providers exempt from apiKey) to issue an LLM call. Used as the
// "configured-but-not-test-passed" fallback so users don't have to click
// "Test Connection" before any command works.
...
describe('hasLLMConfig', () => {
  ...
  it('returns true when provider, apiKey, and model are all set (cloud provider)', () => {
    expect(hasLLMConfig(baseSettings)).toBe(true);
  });

  it('returns true for ollama even without an apiKey', () => {
    const s = { ...baseSettings, provider: 'ollama' as const, apiKey: '', model: 'llama3' };
    expect(hasLLMConfig(s)).toBe(true);
  });

  it('returns true for lmstudio even without an apiKey', () => {
    const s = { ...baseSettings, provider: 'lmstudio' as const, apiKey: '', model: 'qwen2.5' };
    expect(hasLLMConfig(s)).toBe(true);
  });

  it('returns false when provider is empty', () => {
    const s = { ...baseSettings, provider: '' };
    expect(hasLLMConfig(s)).toBe(false);
  });

  it('returns false when model is empty', () => {
    const s = { ...baseSettings, model: '' };
    expect(hasLLMConfig(s)).toBe(false);
  });

  it('returns false when apiKey is missing for a cloud provider', () => {
    const s = { ...baseSettings, apiKey: '' };
    expect(hasLLMConfig(s)).toBe(false);
  });

  it('returns false when apiKey is only whitespace for a cloud provider', () => {
    const s = { ...baseSettings, apiKey: '   ' };
    expect(hasLLMConfig(s)).toBe(false);
  });

  it('returns false when model is only whitespace', () => {
    const s = { ...baseSettings, model: '   ' };
    expect(hasLLMConfig(s)).toBe(false);
  });

  it('ignores llmReady flag — config completeness is independent', () => {
    // llmReady is the upstream "test passed" gate; hasLLMConfig only inspects
    // the user's input. requireLLMReady consults llmReady first, then falls
    // back to this function, so the two are layered not duplicated.
    expect(hasLLMConfig({ ...baseSettings, llmReady: true })).toBe(true);
    expect(hasLLMConfig({ ...baseSettings, llmReady: false })).toBe(true);
  });
});
