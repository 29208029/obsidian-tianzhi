...
export interface LLMWikiSettings {
  ...
  // Sidebar — which tab is active in the right-side sidebar ItemView.
  // Persisted so reopening the plugin keeps the user where they left off.
  activeSidebarTab?: 'chat' | 'maintain' | 'sync';
...
export const PREDEFINED_PROVIDERS: Record<string, ProviderConfig> = {
  ...
  cosmogpt: {
    ...
    nameEn: 'Tianzhi LLM',
    nameZh: '天智大模型',
    // Note: https://gpt.cosmoplat.com exposes the Anthropic Messages API
    // (POST /v1/messages with x-api-key + Anthropic-Version headers), so
    // the actual provider after login is `anthropic-compatible` — see
    // `cosmoGptBootstrap` step [2/4]. This cosmogpt entry is a marker
    // shown in the provider dropdown that triggers the bootstrap pipeline
    // when picked; it doesn't drive the runtime LLM client.
    baseUrl: 'https://gpt.cosmoplat.com',
    apiKeyPlaceholder: '从工号获取 (fetch via employee code)',
    apiKeyPlaceholderEn: 'Fetch via employee code',
    apiKeyPlaceholderZh: '从工号获取',
    requiresBaseUrl: false
  },
  glm: {
    id: 'glm',
