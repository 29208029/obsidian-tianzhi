...
const SETTINGS_LOG_TAG = '[cosmo-bootstrap:settings]';
// eslint-disable-next-line obsidianmd/rule-custom-message
const settingsLog = (...args: unknown[]): void => console.log(SETTINGS_LOG_TAG, ...args);
const settingsError = (...args: unknown[]): void => console.error(SETTINGS_LOG_TAG, ...args);
...
export class LLMWikiSettingTab extends PluginSettingTab {
  ...
  /**
   * Re-sync `tempSettings` from `plugin.settings` and re-render the
   * settings panel. Called by the CosmoGPT bootstrap after it writes
   * apiKey / baseUrl / model into `plugin.settings` — without this
   * the user would see an empty API Key input even though the
   * bootstrap actually succeeded.
   *
   * Always re-renders (`display()`) so the freshly written values
   * appear immediately, even if the bootstrap ran while the panel
   * was hidden (e.g. user clicked "Sign In" in the sidebar and the
   * settings tab was never opened during the bootstrap). When the
   * panel isn't shown, Obsidian no-ops the DOM updates harmlessly.
   */
  refreshFromPluginSettings(): void {
    ...
    this.tempSettings = {
      ...this.plugin.settings,
      watchedFolders: [...(this.plugin.settings.watchedFolders || [])],
    };
    this.display();
  }
  ...
  display() {
    ...
    new Setting(containerEl)
      ...
      .addDropdown(dropdown => {
        ...
        dropdown.onChange((value) => {
          ...
          // Auto-trigger CosmoGPT bootstrap when switching TO cosmogpt
          // provider. This covers the scenario where the user was already
          // logged in to IAM (employee_code exists) but hadn't selected
          // cosmogpt yet.
          if (value === 'cosmogpt') {
    ...
    // CosmoGPT (天智大模型) quick login — only shown when provider is cosmogpt.
// Auto-runs the IAM key-exchange the moment the user finishes typing their
// employee code (debounced). After a successful exchange it auto-populates
// apiKey + baseUrl, refreshes the model list, and selects the default
// model (COSMO-Mind-think). No explicit button required.
if (this.tempSettings.provider === 'cosmogpt') {
      ...
      new Setting(containerEl)
        ...
        .setDesc(loggedIn
          ? this.getText('cosmoSectionDescReady')
          : this.getText('cosmoSectionDescLogin'))
        .setHeading();

      const statusSetting = new Setting(containerEl)
        .setName(this.getText('cosmoStatusName'))
        .setDesc(loggedIn
          ? this.getText('cosmoStatusReady').replace('{}', String(iamState.userInfo?.employee_code ?? ''))
          : this.getText('cosmoStatusNotReady'));
      if (!loggedIn) {
  ...
  /**
   * Fetch the available model list from the current provider's API and
   * write it into `tempSettings.availableModels`.
   *
   * Shared by the "Fetch Models" button and the CosmoGPT quick-login flow
   * (which auto-calls this after writing the freshly exchanged API key).
   * Side effects: mutates `tempSettings.availableModels`, `tempSettings.model`,
   * `tempSettings.useCustomModel`; shows success/error Notices.
   *
   * Throws on transport errors so callers (e.g. CosmoGPT quick-login) can
   * decide whether to swallow or surface the failure — the standalone
   * button already catches and reports.
   */
  private async fetchAvailableModels(): Promise<void> {
    ...
    // Smart filter: OpenRouter keeps '/', Ollama keeps ':', others strip both.
    const getModelFilter = (provider: string) => {
      if (provider === 'openrouter') return (id: string) => !id.includes(':');
      if (provider === 'ollama') return (id: string) => !id.includes('/');
      return (id: string) => !id.includes(':') && !id.includes('/');
    };
