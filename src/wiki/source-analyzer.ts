...
import { parseJsonResponse, matchExtractedToExisting, coerceToArray, getText } from '../utils';
...
export class SourceAnalyzer {
  ...
  async analyzeSource(file: TFile): Promise<SourceAnalysis | null> {
    ...
    for (let batchNum = 0; batchNum < limits.maxBatches; batchNum++) {
      ...
      const lang = this.ctx.settings.language;
      if (isFirstBatch) {
        this.ctx.onProgress?.(getText(lang, 'sourceAnalyzingBatch')
          .replace('{current}', '1')
          .replace('{total}', String(limits.maxBatches)));
      } else {
        this.ctx.onProgress?.(getText(lang, 'sourceAnalyzingBatchContinuing')
      ...
      try {
        ...
        this.ctx.onProgress?.(getText(this.ctx.settings.language, 'sourceAnalyzedBatch')
          .replace('{current}', String(batchNum + 1)));
